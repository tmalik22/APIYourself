...
class HubSpotConnector {
    ...
    async exchangeCodeForToken(code) {
        try {
            const response = await axios_1.default.post('https://api.hubapi.com/oauth/v1/token', {
    ...
    async getSchema(accessToken) {
        try {
            ...
            const [accountInfo, contactsResponse] = await Promise.all([
                axios_1.default.get('https://api.hubapi.com/account-info/v3/details', {
                    headers: { 'Authorization': `Bearer ${accessToken}` }
                }),
                axios_1.default.get('https://api.hubapi.com/crm/v3/objects/contacts?limit=1', {
                    headers: { 'Authorization': `Bearer ${accessToken}` }
                })
            ]);
            return {
