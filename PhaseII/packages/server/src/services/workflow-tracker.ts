...
// Workflow step interface for detailed tracking
...
export class WorkflowTracker {
  private activeSteps = new Map<string, WorkflowStep>();
  private completedSteps: WorkflowStep[] = [];
  private readonly maxStoredSteps = 10000;
  ...
  completeStep(stepId: string, result?: any): void {
    ...
    logger.info('Workflow step performance', {
  ...
  failStep(stepId: string, error: string | Error): void {
    ...
    logger.error('Workflow step failed performance', {
   ...
   */
  getStepsByOperation(operation: string, limit = 100): WorkflowStep[] {
  ...
  getWorkflowAnalysis(): {
    ...
    for (const [operation, stats] of Object.entries(operationStats)) {
      ...
      typedStats.averageDuration = typedStats.durations.length > 0 
        ? typedStats.durations.reduce((a: number, b: number) => a + b, 0) / typedStats.durations.length 
        : 0;
   ...
   */
  getCompletedSteps(limit = 1000): WorkflowStep[] {
    return this.completedSteps.slice(-limit);
  }

  private addCompletedStep(step: WorkflowStep): void {
  ...
  private generateRecommendations(operation: string, stats: any): string[] {
    ...
    if (stats.commonErrors.some((error: string) => error.includes('timeout'))) {
      recommendations.push('Increase timeout limits or optimize external API calls');
    }

    if (stats.commonErrors.some((error: string) => error.includes('API key') || error.includes('authentication'))) {
      recommendations.push('Check API key configuration and authentication setup');
    }

    if (stats.commonErrors.some((error: string) => error.includes('rate limit'))) {
