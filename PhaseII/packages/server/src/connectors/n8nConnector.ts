...
export class N8nConnector implements Connector {
  providerId = 'n8n';
   ...
   */
  async triggerWorkflow(webhookUrl: string, data: any, options: any = {}): Promise<any> {
    try {
   ...
   */
  async getWorkflowStatus(n8nApiUrl: string, executionId: string, apiKey?: string): Promise<any> {
    try {
      ...
      const response = await axios.get(
        `${n8nApiUrl}/api/v1/executions/${executionId}`,
        { headers }
      );
   ...
   */
  async handleWebhookFromN8n(req: Request, res: Response): Promise<void> {
    try {
      const data = req.body;
